...
class Barbarian:
    ...
    def move(self, King, pos, V, type):
        ...
        if(r + c == 1):
            ...
            if(info == pt.TOWNHALL):
                self.break_building(King,pos[0], pos[1], V)
                return
            x = int(info.split(':')[1])
            y = int(info.split(':')[2])
            self.break_building(King,x, y, V)
            return
        elif type == 1:
            flag = 0
    ...
    def break_wall(self, King,x, y, V):
        target = V.wall_objs[(x, y)]
        self.attack_target(King, target, 0)

    def break_building(self, King, x, y, V):
        target = None
        ...
        self.attack_target(King, target, 1)

    def attack_target(self, King, target, type):
        if(self.alive == False):
            return
...
class StealthArcher:
    def __init__(self, position):
        ...
        self.alive = True
        self.target = None
        self.update = threading.Thread(target=self.updateStealth)

    def updateStealth(self):
        if (not self.stealth):
            return
        time.sleep(10)
        self.stealth = False
...
class Healer:
    ...
    def move(self, King, pos, V):
        ...
        if(self.isInHealradius(pos)):
            self.heal_target_area(King, pos)
            return
        elif(r == 0):
            if(pos[1] > self.position[1]):
                for i in range(self.speed):
            ...
            if(pos[0] > self.position[0]):
                for i in range(self.speed):
                    ...
                    c = self.position[1]
                    self.position[0] += 1
                    if(self.position[0] == pos[0] or self.isInHealradius(pos)):
                        return
            else:
                for i in range(self.speed):
                    r = self.position[0] - 1
            ...
            if(pos[1] > self.position[1]):
                for i in range(self.speed):
                    ...
                    c = self.position[1] + 1
                    self.position[1] += 1
                    if(self.position[1] == pos[1] or self.isInHealradius(pos)):
                        return
            else:
                for i in range(self.speed):
                    r = self.position[0]
    ...
    def heal_troops(self, King):
        troops = barbarians+ archers + dragons + balloons + healers + s_archers + [King]
        for troop in troops:
            if (troop.position[0] - self.position[0])**2 + (troop.position[1] - self.position[1])**2 <= self.heal_radius**2:
                self.heal_target_area(King, troop)
                return

    def heal_target_area(self, King, target):
        if(self.alive == False):
            return
        
        i = target[0] - 1
        j = target[1] - 1
        troops = barbarians+ archers + dragons + balloons + s_archers + healers + [King]
...
def spawnS_Archer(pos):
    if(pt.troop_limit['s_archer'] <= troops_spawned['s_archer']):
        return

    # convert tuple to list
    pos = list(pos)
    s_archer = StealthArcher(pos)
    troops_spawned['s_archer'] += 1
    s_archers.append(s_archer)
    s_archer.update.start()
...
def move_barbarians(King, V, type):
    if(type == 1):
        for barb in barbarians:
            ...
            if(barb.target == None):
                continue
            barb.move(King, barb.target, V, type)
    elif(type == 2):
        for barb in barbarians:
            if(barb.alive == False):
                continue
            ...
            if(closest_building == None):
                continue
            barb.move(King, closest_building, V, type)

def move_archers(King, V, type):
    if(type == 1):
        for archer in archers:
            ...
            if(archer.target == None):
                continue
            archer.move(King, archer.target, V,type)
    elif(type == 2):
        for archer in archers:
            if(archer.alive == False):
                continue
            ...
            if(closest_building == None):
                continue
            archer.move(King, closest_building, V, type)

def move_s_archers(King, V, type):
    if(type == 1):
        for archer in s_archers:
            if(archer.alive == False):
                continue
            if archer.target != None:
                if(V.map[archer.target[0]][archer.target[1]] == pt.BLANK):
...
def move_healers(King, V):
    for hlr in healers:
        if(hlr.alive == False):
            continue
        closest_troop = search_for_closest_in_need_troop(King, hlr.position)
        if(closest_troop == None):
            continue
        hlr.move(King, closest_troop, V)
...
def search_for_closest_in_need_troop(King, pos):
    closest_troop = None
    closest_dist = 10000
    flag = 0

    troops = barbarians + archers + s_archers + dragons + balloons + [King]
    for troop in troops:
