...
def s_archColor(s_arch):
    health = s_arch.health
    max_health = s_arch.max_health
    percentage = (health*100)/max_health
    if (s_arch.stealth):
        return Back.BLACK
    if(percentage > 50):
...
def printMap(V):
    ...
    for s_arch in s_archers:
        a= 2*s_arch.position[0]
        b = 2*s_arch.position[1]
        map_matrix[a][b] = s_archColor(s_arch) + Fore.BLACK + 'S_'
        map_matrix[a+1][b] = s_archColor(s_arch) + Fore.BLACK + 'CH'
        map_matrix[a][b+1] = s_archColor(s_arch) + Fore.BLACK + 'AR'
        map_matrix[a+1][b+1] = s_archColor(s_arch) + Fore.BLACK + 'ER'
    
    for hlr in healers:
        a= 2*hlr.position[0]
        b = 2*hlr.position[1]
        map_matrix[a][b] = hlrColor(hlr) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = hlrColor(hlr) + Fore.BLACK + 'ER'
        map_matrix[a][b+1] = hlrColor(hlr) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = hlrColor(hlr) + Fore.BLACK + '  '
